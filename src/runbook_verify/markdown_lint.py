...
RULES: tuple[PhraseRule, ...] = (
    ...
    PhraseRule(
        ...
        "Prose handles credentials without an executable credential rotation or revocation model.",
        "Model credential state with rotate_credential/revoke_credential and credential_active or credential_revoked effects, or document an explicit limitation.",
    ),
    PhraseRule(
        "unsafe-retry-needs-effect-annotation",
        "warning",
        re.compile(r"\b(retry|rerun|repeat|run again)\b.{0,80}\b(delete|drain|failover|flush|revoke|rotate|replay|sql|migration|dns|credential|key)\b|\b(delete|drain|failover|flush|revoke|rotate|replay|sql|migration|dns|credential|key)\b.{0,80}\b(retry|rerun|repeat|run again)\b", re.I),
        None,
        (),
        "unsafe_retry_annotation",
        "Prose encourages retrying a high-risk operation without an executable idempotency/reversibility annotation.",
        "Add effect_annotations with idempotency, reversibility, retry_safety, blast_radius, and expected_user_impact, or make retry guidance conditional.",
    ),
    PhraseRule(
