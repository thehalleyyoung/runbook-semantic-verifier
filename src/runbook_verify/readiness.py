...
def build_readiness_report(path: str | Path, options: ReadinessOptions | None = None) -> dict[str, Any]:
    ...
    for file in executable_paths:
        ...
        if _matches_scope(runbook, options):
            checked.append(_CheckedRunbook(file, runbook, Checker(runbook).check(), _expected_labels(doc)))
    ...
    summary = _summary(checked, parse_errors, semantic_findings, prose_findings, stale_files, missing_rollback, coverage_findings, benchmark_mismatches)
    return {
        "path": str(root),
        ...
        "stale_preconditions": stale_files,
...
def render_readiness_markdown(report: dict[str, Any]) -> str:
    ...
    if report["stale_preconditions"]:
        ...
        lines.append("No file exceeded the configured freshness window.")
...
def _summary(
    ...
    not_ready = bool(parse_errors or semantic_findings or blocking_prose or coverage_findings or benchmark_mismatches)
    advisory = bool(warnings or stale_files or missing_rollback or prose_findings)
    ...
    return {
        ...
        "stale_preconditions": len(stale_files),
