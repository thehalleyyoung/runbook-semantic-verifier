...
def export_tla(runbook: Runbook) -> str:
    ...
    lines = [
        ...
        "VARIABLES done, services, databases, alerts, flags",
        "",
        "Init == /\\ done = <<>>",
        f"        /\\ Steps = {{{step_names}}}",
        "",
        ...
        "             /\\ UNCHANGED <<services, databases, alerts, flags>>",
        "",
        ...
        "Safety == NoAllReplicasDrained /\\ BoundedAlertSuppression /\\ NoUnsafeFailover",
        "",
        "Spec == Init /\\ [][Next]_<<done, services, databases, alerts, flags>>",
        "THEOREM Spec => []Safety",
        "====",
    ]
    lines.append("\\* Action mapping from DSL:")
    for step in runbook.steps:
        lines.append(f"\\* {step.id}: {step.action} {step.params}")
    return "\n".join(lines) + "\n"
...
def export_alloy(runbook: Runbook) -> str:
    ...
    for step in runbook.steps:
        lines.append(f"one sig { _symbol(step.id) } extends Step {{}} // {step.action}")
    lines.extend([
