...
def export_tla(runbook: Runbook) -> str:
    ...
    lines = [
        ...
        "VARIABLES done, services, databases, alerts, flags, trafficRoutes, dnsRecords",
        "",
        "Init == /\\ done = <<>>",
        f"        /\\ Steps = {{{step_names}}}",
        "",
        ...
        "             /\\ UNCHANGED <<services, databases, alerts, flags, trafficRoutes, dnsRecords>>",
        "",
        ...
        "Safety == NoAllReplicasDrained /\\ BoundedAlertSuppression /\\ NoUnsafeFailover /\\ TrafficWeightsNormalized /\\ DNSNoSplitBrain",
        "",
        "Spec == Init /\\ [][Next]_<<done, services, databases, alerts, flags, trafficRoutes, dnsRecords>>",
        "THEOREM Spec => []Safety",
        "====",
    ]
