# Proof obligations: Grafana Tempo current public runbook derived safety model
...
| `guarantee:tempo-query-fallback-contract:1` | assume-guarantee-guarantee | `tempo-query-fallback-contract` | {"depth": 18000, "kind": "queue_depth_at_most", "queue": "tenant-index-fallback-scan"} | native checker at initial state and after contracted steps |
| `rely-action:background-tenant-index-writes:1` | rely-guarantee-interference-action | `background-tenant-index-writes` | {"action": "replay_messages", "params": {"count": 1, "queue": "tenant-index-fallback-scan"}} | native checker as one-step external actor interference |
| `rely-preserves:background-tenant-index-writes:1` | rely-guarantee-preservation | `background-tenant-index-writes` | {"depth": 18000, "kind": "queue_depth_at_most", "queue": "tenant-index-fallback-scan"} | native checker before each applicable runbook action |
