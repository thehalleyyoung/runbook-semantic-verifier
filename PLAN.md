...
- `frv annotate`: implemented pull-request annotations that group semantic/prose findings by formal obligation and source span, with GitHub Actions commands plus JSON/Markdown reviewer summaries.
...
- `frv readiness`: implemented service or repository incident-readiness report covering validation, bounded-checking status, audit severity, stale file and inventory-refinement preconditions, rollback/restore coverage, proof obligations, and uncovered service/region paths.
...
- `frv profiles`: implemented named exit-policy profiles for conservative production gates, advisory research review, documentation-only audits, and benchmark reproduction. Profiles set defaults only when `--fail-on` is omitted and do not alter semantic findings.
